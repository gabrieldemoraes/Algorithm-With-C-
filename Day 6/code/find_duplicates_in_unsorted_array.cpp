...
std::vector<int> find_duplicates_in_unsorted_array_1(std::vector<int>& xs)
{
    ...
    //O(N) * O(N) = O(N^2)
    for (std::size_t i = 0; i < N; ++i) //O(N)
    {
        for (std::size_t j = i + 1; j < N; ++j) //O(N)
        {
            if (xs[i] == xs[j])
            {
...
std::vector<int> find_duplicates_in_unsorted_array_2(std::vector<int>& xs)
{
    ...
    std::sort(xs.begin(), xs.end()); //O(N lg N)

    for (std::size_t i = 1; i < N; ++i) //O(N)
    {
        if (xs[i - 1] == xs[i])
        {
            result.emplace_back(xs[i]);
...
{
    std::vector<int> result;
    std::unordered_set<int> presence; //N * O(1) == O(N) //auxiliar
