...
{
    void operator()(FILE* file_ptr) { fclose(file_ptr); };
};
...
namespace raii
{
    ...
    //Using RAII idiom (lambda as deleter)
    static void write_strings_to_file_raii_heap_1(const char* filename, std::vector<std::string> ss, size_t& written)
    {
        //heap
        auto fclose_deleter = [](FILE* file_ptr) { fclose(file_ptr); };
    ...
    //Using RAII idiom (std::default_delete specialization as deleter)
    static void write_strings_to_file_raii_heap_2(const char* filename, std::vector<std::string> ss, size_t& written)
    {
        std::unique_ptr<FILE> file_ptr(deprecated_fopen(filename, "w+"));
        if (file_ptr)
        {
            for (auto& s : ss)
            {
